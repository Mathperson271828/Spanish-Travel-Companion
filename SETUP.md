...
## 🧪 Instructions for Graders (Testing)

To verify the system is functioning correctly, please follow these steps:

1. **Check Backend Connection:** Visit `http://localhost:10000/docs` to view the FastAPI Swagger UI and ensure the server is running.
2. **Voice Activity Detection (VAD) Test:** Click the "Start Speaking" button on the frontend. The UI should visually indicate it is recording and use the **Web Audio API** to detect when you stop speaking (3 seconds of silence).
3. **End-to-End Chat Test:**
   * Speak a simple Spanish phrase (e.g., *"¿Dónde está la estación de tren?"*).
   * Verify that **Whisper-1** transcribes the text correctly.
   * Confirm that the bot responds in Spanish at the selected difficulty level (Beginner/Intermediate).
   * Listen for the audio playback generated by the **TTS-1** model.
4. **Error Handling:** If an API key is missing or invalid, the interface should return an error message prompting you to check your API key.
